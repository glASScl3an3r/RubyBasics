...
class Interface
  ...
  #interfice only provides a start method, others used for simplify the code
  private
  ...
  def create_train(train_serial, str_type)
    current_type = (str_type == 'p' ? :passenger : :cargo)

    @trains[train_serial] = Train.new(train_serial, current_type)
  end
